...
// PERBAIKAN: Path import disesuaikan ke data_pasien
import {
  getPatients,
  getPatientById,
  updatePatient,
    ...
    <div className="fixed inset-0 bg-black/30 z-50 flex items-center justify-center">
      <div className="bg-white rounded-lg shadow-lg w-full max-w-2xl max-h-[90vh] flex flex-col relative">
        <div className="p-4 border-b border-[#81B7A9] flex justify-between items-center">
          <h2 className="text-lg font-semibold text-[#36315B]">Detail Patient</h2>
          <button onClick={onClose} className="text-gray-600 hover:text-[#81B7A9]">✕</button>
        </div>
...
export default function PatientPage() {
  ...
  return (
    <div className="flex min-h-screen bg-gray-50">
            ...
            <table className="w-full text-sm border-collapse">
              ...
              <tbody>
                {filtered.map((pasien, i) => (
                  <tr key={pasien.child_id} className="border-b border-[#81B7A9] hover:bg-gray-50">
                    ...
                    <td className="p-3 flex justify-center gap-3">
                      <button onClick={() => handleDetail(pasien.child_id)} className="hover:scale-110 transition text-[#36315B]">
                        <Eye size={18} />
                      </button>
                      <button
                        onClick={async () => {
                      ...
                      <button onClick={() => { setDeleteId(pasien.child_id); setShowHapus(true); }} className="hover:scale-110 transition text-red-600">
                        <Trash2 size={18} />
                      </button>
                    </td>
                  </tr>
      ...
      />
      <DetailPatient open={showDetail} onClose={() => setShowDetail(false)} patient={selectedPatient} />
    </div>
  );
}