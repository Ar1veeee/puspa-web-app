  ...
  return (
    <ResponsiveOrangtuaLayout maxWidth="max-w-7xl">
      <div className="space-y-6 md:space-y-10 text-[#36315B]">
        ...
        {/* CHART SECTION */}
        <div className="bg-white rounded-2xl p-5 md:p-8 border border-gray-100 shadow-[0_8px_20px_-5px_rgba(16,185,129,0.20)]">
          ...
          <div className="h-70 sm:h-80 md:h-87.5 w-full">
            <ResponsiveContainer width="100%" height="100%">
              <AreaChart data={customChartData} margin={{ top: 10, right: 10, left: -20, bottom: 0 }}>
                ...
                <XAxis dataKey="name" axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#64748B' }} dy={15} />
                <YAxis axisLine={false} tickLine={false} tick={{ fontSize: 10, fill: '#94A3B8' }} domain={[0, 100]} ticks={[0, 20, 40, 60, 80, 100]} />
                <ReTooltip contentStyle={{ borderRadius: '12px', border: 'none' }} />
                <Area type="monotone" dataKey="value" stroke="#81B7A9" strokeWidth={2} fill="url(#colorValue)" dot={{ r: 4, fill: '#fff', stroke: '#81B7A9' }} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        {/* JADWAL SECTION */}
        <div className="bg-white rounded-2xl p-5 md:p-8 mb-10 border border-gray-100 shadow-[0_8px_20px_-5px_rgba(16,185,129,0.20)]">
          <div className="flex flex-col lg:flex-row lg:items-center justify-between gap-5 mb-8">
            ...
            <div className="flex flex-col sm:flex-row gap-4 items-stretch sm:items-center">
              <div className="relative group flex-1 sm:flex-initial">
                <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-gray-400 group-focus-within:text-[#81B7A9]" />
                <input
                  value={q}
                  onChange={(e) => setQ(e.target.value)}
                  placeholder="Cari nama anak..."
                  className="w-full sm:w-64 pl-10 pr-4 py-2.5 bg-gray-50 border-none rounded-xl text-sm outline-none focus:ring-2 focus:ring-[#81B7A9]/20"
                />
              </div>
              ...
              <div className="flex bg-gray-100 p-1 rounded-xl overflow-x-auto no-scrollbar">
                {["Semua", "Observasi", "Assessment"].map((t) => (
                  <button
                    ...
                    className={`whitespace-nowrap px-4 md:px-6 py-2 rounded-lg text-xs md:text-sm font-bold transition-all ${activeTab === t ? "bg-white text-[#4A8B73] shadow-sm" : "text-gray-500 hover:text-gray-700"} cursor-pointer`}
                  >
                    {t}
                  </button>
                ))}
            ...
            <table className="min-w-full">
              ...
              <tbody className="divide-y divide-gray-50">
                {filteredSchedule.length === 0 ? (
                  <tr><td colSpan={6} className="py-16 text-center text-gray-400">Belum ada jadwal yang terdaftar</td></tr>
                ) : (
                  filteredSchedule.map((r, index) => (
                    <tr key={`desktop-${r.jenis}-${r.id}-${index}`} className="group hover:bg-gray-50/50 transition-colors">
                      <td className="py-5 px-2 font-bold text-gray-700 group-hover:text-[#4A8B73]">{r.nama_pasien}</td>
          ...
          <div className="block sm:hidden space-y-4">
            {filteredSchedule.length === 0 ? (
              ...
              filteredSchedule.map((r, index) => (
                <div key={`mobile-${r.jenis}-${r.id}-${index}`} className="bg-gray-50/50 border border-gray-100 p-4 rounded-2xl space-y-3">
...
function MetricCard({ title, data, icon, color }: any) {
  return (
    <div className="bg-white rounded-2xl p-5 sm:p-6 border border-gray-100 flex justify-between items-center transition-all shadow-[0_8px_20px_-5px_rgba(16,185,129,0.20)] group hover:-translate-y-1">
      ...
      <div className={`${color} w-12 h-12 md:w-16 md:h-16 flex items-center justify-center rounded-2xl group-hover:scale-110 transition-transform`}>
