...
export default function RiwayatJawabanUmum() {
  ...
  const renderAnswer = (answer: any): React.ReactNode => {
    ...
    if (typeof answer === "object" && !Array.isArray(answer) && "value" in answer) {
      return <span className="font-semibold text-gray-800">{String(answer.value ?? "-")}</span>;
    }
    ...
    if (Array.isArray(answer)) {
      ...
      if (typeof answer[0] !== "object") {
        return (
          <div className="flex flex-wrap gap-2">
            {answer.map((v, i) => (
              <span key={i} className="inline-flex items-center bg-teal-50 text-teal-800 text-xs font-semibold px-2.5 py-1 rounded-lg border border-teal-100/30">
                {String(v)}
              </span>
            ))}
          </div>
      ...
      if (typeof answer[0] === "object") {
        ...
        return (
          <div className="overflow-x-auto border border-teal-50/60 rounded-xl bg-white shadow-sm">
            <table className="min-w-full text-xs text-left">
              <thead className="bg-[#EAF4F2]/50 text-[#1E5C58]">
                <tr>
                  {headers.map((h) => (
                    <th key={h} className="px-4 py-2.5 font-bold uppercase tracking-wider border-b border-teal-50">
                      {h}
                    </th>
                  ))}
                </tr>
              ...
              <tbody className="divide-y divide-teal-50/50">
                {answer.map((row, i) => (
                  <tr key={i} className="hover:bg-teal-50/10 transition-colors">
                    {headers.map((h) => (
                      <td key={h} className="px-4 py-2 text-gray-700 font-medium">
                        {String(row[h] ?? "-")}
                      </td>
                    ))}
                  </tr>
      ...
      <div className="bg-white rounded-3xl border border-teal-50/60 shadow-[0_4px_24px_rgba(30,92,88,0.02)] max-w-7xl mx-auto overflow-hidden">
        ...
        <div className="bg-[#EAF4F2]/30 px-6 py-4 border-b border-teal-50 flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex items-center gap-2 text-[#1E5C58]">
            ...
            <h2 className="text-sm font-extrabold">{activeAspek.label}</h2>
          </div>
